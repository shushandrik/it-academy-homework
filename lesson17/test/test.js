...
describe('Site ChromeDrivers test', () => {
  let driver;
  before(async () => {
  ...
  it('2. Title should contains "Chrome Extensions" and highlighting ', async () => {
    ...
    driver.executeScript("arguments[0].style.backgroundColor = 'red'", h1HighLight);
    const compareTitle = await driver.getTitle();
  ...
  it('3. The search field should contains "driver"', async () => {
    ...
    expect(await searchField.getText()).to.contain('driver'); /* expect(await searchField.getText().toLowerCase()).to.contain('driver'); ---- с ним не работает (TypeError: searchField.getText(...).toLowerCase is not a function) */
  });
